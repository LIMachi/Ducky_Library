...
public class Inventories {
    public static class ItemStackIORights {
        ...
        public CompoundNBT writeNBT(CompoundNBT nbt) {
            ...
            if (stacks.size() != 0) nbt.put("Stacks", NBTs.convertList(stacks, null, t->t.write(new CompoundNBT())));
            return nbt;
        }
        ...
        public void readNBT(CompoundNBT nbt) {
            if (nbt == null) nbt = new CompoundNBT();
            flagsFromByte(NBTs.getOrDefault(nbt, "Flags", nbt::getByte, VANILLA.flagsAsByte()));
            minStack = NBTs.getOrDefault(nbt, "Min", nbt::getByte, VANILLA.minStack);
            maxStack = NBTs.getOrDefault(nbt, "Max", nbt::getByte, VANILLA.maxStack);
            ListNBT list = nbt.getList("Tags", 10);
            tags = new ArrayList<>();
            if (list.size() != 0) {
                tags.ensureCapacity(list.size());
                NBTs.populateList(list, tags, null, t -> ItemTags.getCollection().getTagByID(new ResourceLocation(t.getString("TagResourceLocation"))));
            }
            ...
            if (list.size() != 0) {
                ...
                NBTs.populateList(list, stacks, null, ItemStack::read);
            }
        }
    ...
    public interface IIORIghtItemHandler extends IItemHandler {
        ...
        void setRightsInSlot(int slot, Inventories.ItemStackIORights right);
    }
