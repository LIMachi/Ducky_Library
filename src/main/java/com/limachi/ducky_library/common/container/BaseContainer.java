...
import com.limachi.ducky_library.common.container.slots.IIORightsSlot;
import com.limachi.ducky_library.common.network.PacketHandler;
import com.limachi.ducky_library.common.network.packets.FluidSlotSyncMsg;
import com.limachi.ducky_library.common.network.packets.TrackedStringSyncMsg;
...
public class BaseContainer extends Container {
    ...
    public BaseContainer(ContainerType<? extends BaseContainer> type, int windowId, PlayerInventory playerInv, PacketBuffer extraData) {
        ...
        switch (this.connectionType) {
            case TILE_ENTITY:
                this.tileEntity = Worlds.getWorld(null, extraData.readString()).getTileEntity(extraData.readBlockPos());
                this.itemSlot = null;
                break;
            case ITEM:
                this.tileEntity = null;
    ...
    public static PacketBuffer writeBaseParameters(PacketBuffer buffer, ContainerConnectionType connectionType, TileEntity tileEntity, int itemSlot) {
        ...
        if (connectionType == ContainerConnectionType.TILE_ENTITY) {
            buffer.writeString(Worlds.worldRKToString(tileEntity.getWorld().getDimensionKey()));
    ...
    public void loadFluidSlotChange(int slotIndex, FluidStack fluid) {
        ...
        FluidSlot slot = remoteFluidSlots.get(slotIndex);
        slot.getFluidHandler().setFluidInTank(slot.slotNumber, fluid);
    }
...
//            blackList.add(getSameSlotIndex(slot)); //FIXME: add back a protection for the wrapped slots
            ...
            slot.putStack(itemStack = Inventories.mergeItemStack(inventorySlots, slotStack, 0, inventorySlots.size(), false, blackList));
            slot.onSlotChanged();
        }
        return itemStack;
    }
    ...
    public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, PlayerEntity player) {
        ...
        if (clickTypeIn == ClickType.CLONE && slot instanceof IIORightsSlot) {
            ((IIORightsSlot)slot).nextRights();
            return ItemStack.EMPTY;
        }
        if (slot instanceof FluidSlot) return ((FluidSlot)slot).onSlotClick(player, clickTypeIn);
